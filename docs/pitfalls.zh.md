...
> 当前发布：**v5.2（诊断灯终版：红/灭/恢复绿闪）**；v5/v5.1 为废案已撤下 releases 与 tags；v4/v3.3 保留（A/B 命名自 v5 起，v3.3=A 线、v4=B 线）。核心功能 v3.3 全量实测通过。
...
| **v5**（废案，已撤下） | 链路健康自诊断：`0xF1` RPC（每秒、互报充电态）+ 两侧 `7` 键灯（绿=数据好/红闪=数据断/黄闪=有线一半没5V）；A/B 同源码两版（`LINK_WATCH_ALWAYS=yes`） |
| **v5.1**（废案，已撤下） | 实测修正架构（单链、主半画全灯、右半纯靠链路 5V）后：诊断灯挪至左右 Ctrl、撤销绿灯改**状态机**（闪3下→常亮、恢复绿闪3下、开机即坏直接常亮）、黄闪仅主半判、全部提示灯亮度 +1 档封顶 |
...
## 6.6 二次复盘：黄灯的诞生与死亡（2026-09-16）

设计「供电断」黄灯时，我假设从半 `HS_BAT_CABLE_PIN(A7)` 感知链路 5V。用户实测一票否决：好线、USB、通信正常也常黄——从半 A7 悬空恒 0，与主半恒 1 永远「不对称」。教训：
1. 同一份固件两半都读某 sense 脚 ≠ 两半都真接了那个 sense；
2. 基于电气推断的指示，上线前必须做「已知好线 vs 已知坏线」对照实验；
3. 彻底的限制：WS2812 链单向无回读，「数据好但灯链坏」原理性不可测。诊断灯的诚实边界 = 只报 UART 通断（红），其余交给人眼。v5.2 起黄灯移除。
