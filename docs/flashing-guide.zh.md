...
> 适用固件：**本仓库编译出的修改版**（新增 `F 模式下 ESC→\` / Shift+ESC→~`）。
> 目标：`epomaker/split65:default` ｜ MCU：`WB32FQ95` ｜ 引导：`wb32-dfu`（USB‑DFU 在 **ROM**）
> 设备：VID `0x342D` / PID `0xE4C6`
> 最后更新：2026-09-15（**v4：彻底禁用 LPWR 深睡**→左右两半任意键随按随醒，回到出厂行为；代价=无线待机"数月"→"数周~月"。v3.3 指示亮度联动；v3.2 PrtSc 纯净边沿；v3 ESC↔` 互换+PrtSc；v2 红灯排/Caps红/白闪）
...
| 项目 | 值 |
|---|---|
| 固件文件 | `<QMK>/Split65-firmware/epomaker_split65_default.bin`（同内容也在 `.build/`） |
| 大小 | `74436` 字节 |
| SHA‑256 | `e8118285ba0a89940fa928bc939808061456197a05d8366b11ebecff154cfa5f` |
| 编译产物（备用） | `.build/epomaker_split65_default.hex` / `.elf` |
| 改动清单 (v3) | `keyboards/epomaker/split65/split65.c`：①**ESC↔`` ` `` 双向互换**：F 模式(FILP 开)且未按 Fn 时互换两码，平时所见即所得（详见 §6 表）；②**Shift+Backspace = PrtSc** 截图；③F 模式时 ESC+数字行+`-` `=` 共 13 键亮红灯；④CapsLock/Win 锁指示红色；⑤亮度到顶/到底 ↑/↓ 白闪(修复原"误用速度阈值+最暗右半冻结")；⑥深睡恢复官方：USB 模式永不深睡(修右半久置失灵)；⑦FILP 状态 RPC 同步另一半，跨半红灯排 |
...
- [ ] SHA‑256 与 §0 一致：`e8118285…154cfa5f`。
