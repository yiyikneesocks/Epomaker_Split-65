...
> 最后更新：2026-09-15（**v5：链路健康自诊断灯 + A/B 双版同版本号**；A=深睡保留/仅左半唤醒，B=关深睡/双侧即醒。v4/v3.3 保留在旧 releases）
...
| 版本 | 大小 | SHA‑256 (.bin) |
|---|---|---|
| **v5-A**（单边唤醒/续航优先） | `74796` B | `73379936d8531b2bd5d904828c8c2bfba458bdc62b6da070f80afdd0317e04ce` |
| **v5-B**（双边唤醒/体验优先，推荐） | `74772` B | `2936e47a0446f29ca174eb45a6bbdd345f1542774d0021c7c569dafe5f9906d5` |

产物在 Releases（含 `.hex` 与 `SHA256SUMS.txt`）；自建见 README"自行构建"（同一份源码，B 版加 `LINK_WATCH_ALWAYS=yes`）。
v5 改动 = v4 全部功能 + `0xF1` 链路健康 RPC + 两侧 `7` 键诊断灯（绿/红闪/黄闪，见附录 E）。历史版本功能：v2 红灯排/白闪/Caps 红、v3 ESC↔` 互换 + PrtSc、v3.1 内置默认键位、v3.2 PrtSc 摘 Shift、v3.3 指示亮度联动。
...
- [ ] SHA‑256 与你所选版本一致（§0 表：v5-A `73379936…` / v5-B `2936e47a…`）。
...
4. **F 模式其余**：数字行 = F1~F12；ESC+数字+`-`+`=` 共 13 键亮红灯；再按 Fn+Ctrl 关闭后熄灭。Win/Mac 层一致。**红灯排/白闪/Caps/Win 指示亮度 = 当前亮度+2 档（封顶最亮）**：调暗灯效时指示跟着变暗但最暗时仍可见（不会全黑看不见）。
...
## 附录 E：v5 链路自诊断灯（两侧 `7` 键）

主半每秒向从半发一次 `0xF1` RPC（互报充电状态）。每一半根据自己的视图点亮**自己那侧 `7` 键**（左=LED22，右=LED29）：

| 灯态 | 含义 | 适用版本 |
|---|---|---|
| **常亮柔绿** | 链路数据正常（近 2s 内有成功交换） | A / B 通用 |
| **红色闪（400ms）** | 数据不通：线/触点坏、从半没电、或连接线没插 | **B 任何模式；A 仅有线时**（A 无线下从半本就会深睡，闪红=误报，故抑制） |
| **黄色闪** | 数据好但"一边有 5V 一边没有"=坏线（缺供电）或从半亏电，仅有线模式判定 | A / B 通用 |
| 不亮 | F 模式开启时（让位给 13 键红灯排）；或灯带电源关闭/模式=NONE（所有指示的固有前提） | — |

插线开机 3 秒内即可判断线材好坏：绿=全好；红闪=数据线坏；黄闪=数据线好但供电坏。这正是对"有的线能传数据但不亮灯 / 有的线能亮灯但不传数据"三现象的固件侧显形。
