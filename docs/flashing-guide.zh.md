...
> 最后更新：2026-09-15（**v5.1：诊断灯状态机（左右 Ctrl、闪3下→常亮、恢复绿闪）+ 全部提示灯亮度=灯效+1档封顶**。v5 初版诊断灯已被取代；v4/v3.3 保留）
...
| 版本 | 大小 | SHA‑256 (.bin) |
|---|---|---|
| **v5.1-A**（单边唤醒/续航优先） | `74856` B | `c6ce1034297ced85e514c3bf9654e2d09ca3813ce57b110df2ad73f238a33e30` |
| **v5.1-B**（双边唤醒/体验优先，推荐） | `74848` B | `ee31d8d75f270dc269600813fbf34f75a7661ac86b16d8d91453b8f7a6fd7056` |

产物在 Releases（含 `.hex` 与 `SHA256SUMS.txt`）；自建见 README"自行构建"（同一份源码，B 版加 `LINK_WATCH_ALWAYS=yes`）。
v5.1 改动 = v4 功能 + `0xF1` 链路 RPC + **Ctrl 键诊断状态机（附录 E）** + 全部提示灯亮度=灯效+1档封顶。历史：v5 初版诊断灯（6/7 键、绿常亮）已被 v5.1 取代。历史版本功能：v2 红灯排/白闪/Caps 红、v3 ESC↔` 互换 + PrtSc、v3.1 内置默认键位、v3.2 PrtSc 摘 Shift、v3.3 指示亮度联动。
...
- [ ] SHA‑256 与你所选版本一致（§0 表：v5.1-A `c6ce1034…` / v5.1-B `ee31d8d7…`）。
...
## 附录 E：v5.1 链路诊断灯（左右 Ctrl，状态机）

**架构事实（实测+代码推定）**：整条 RGB 灯链是**单链**——数据由左半（主）MCU 一路画出、经连接线的灯数据导线串到右半灯珠；右半 MCU 只负责按键矩阵/配置应答，**不画灯**。连接线里并行四组线：5V（主半电池升压供右半）、GND、UART（键盘数据）、WS2812 数据线。右半整体由链路 5V 供电（右半无可用电池）。

诊断灯位：**左 Ctrl = LED4，右 Ctrl = LED64**（与 13 键红灯排/Caps/Win 零交集）。每半画自己的状态机（实际显示由主半经链驱动，右半侧故障画面从半无电时自然不可见，语义自洽）。

| 事件 | 表现（红=数据断，黄=有线且"主有5V、从报无5V"，仅主半判黄） |
|---|---|
| 一切正常 | **灭**（v5 的常绿灯已撤销） |
| 开机即异常（只插左半/坏线/右半没电） | 对应色**直接常亮**（无历史可比，不闪） |
| 运行中 正常→异常 | **先闪 3 下（400ms亮/400ms灭）→ 转常亮** |
| 异常种类切换（红↔黄） | 按新色再闪 3 下 → 常亮 |
| 异常→恢复（插回/换好线） | **绿闪 3 下 → 灭** |

判据：2s 内无成功 `0xF1` 交换 = 链路坏（红）。A/B 两版诊断行为一致（单链模型下右半常年在线，"在睡"误报不成立），差异仅剩无线深睡开关。

**坏线三象限速查**：
| 现象 | 结论 |
|---|---|
| 左 Ctrl 红 + 右 Ctrl 也红（能亮但两边打字时好时坏）| 数据线坏、供电好（右半灯效仍由主半驱动照常动）|
| 左 Ctrl 黄 + 右半全黑但右半能打字 | 数据线好、供电线坏（灯带没电）|
| 左 Ctrl 红 + 右半全黑且不能打字 + 线在 | 供电坏且右半 MCU 也没电/线没插到位 |
| 都不亮 | 一切正常 |

提示：灯效模式=NONE 或灯被关闭时，所有指示（含诊断）不显示——QMK 指示钩子固有行为。
## 参考链接
